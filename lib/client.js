...
.pc-noteSaved{color:var(--dsw-alias-state-success-primary, #2fb344);font-size:11px}.pc-verRow{display:flex;align-items:center;gap:8px;flex-wrap:wrap;min-height:22px}.pc-updateBadge{border-radius:999px;padding:1px 8px;font-size:11px;line-height:17px;white-space:nowrap;background:rgba(241,196,15,.12);color:#f1c40f;border:1px solid rgba(241,196,15,.35)}.pc-updateBtn{height:24px;border-radius:6px;border:1px solid rgba(47,179,68,.5);background:rgba(47,179,68,.12);color:#2fb344;cursor:pointer;font-size:12px;padding:0 10px}.pc-updateBtn:hover{background:rgba(47,179,68,.22)}.pc-updateBtn:disabled{opacity:.55;cursor:default}.pc-updateErr{color:#ff8a80;font-size:11px}
...
`
    ...
    function PluginCatalogSection() {
      ...
      const [updateMsg, setUpdateMsg] = useState({}) // name -> {kind:'ok'|'err', text}
      ...
      const loadVersions = useCallback(async () => {
        try {
          const res = await fetch('/plugin-catalog/api/versions', { credentials: 'include' })
          const j = await res.json()
          if (j.ok && Array.isArray(j.versions)) {
            const map = {}
            j.versions.forEach((v) => { map[v.name] = v })
            setVersions(map)
          }
        } catch { /* 版本检查失败不影响列表 */ }
      }, [])
      ...
      const doUpdate = useCallback(async (name) => {
        setUpdating(name)
        setUpdateMsg((m) => ({ ...m, [name]: null }))
        try {
          const res = await fetch('/plugin-catalog/api/update', {
            method: 'POST',
            credentials: 'include',
          ...
          const j = await res.json()
          if (j.ok) {
            setUpdateMsg((m) => ({ ...m, [name]: { kind: 'ok', text: '更新成功，请重启 dsh-web 生效' } }))
          } else {
            setUpdateMsg((m) => ({ ...m, [name]: { kind: 'err', text: j.error || '更新失败' } }))
          }
        } catch (err) {
          setUpdateMsg((m) => ({ ...m, [name]: { kind: 'err', text: '更新失败：' + String(err.message || err) } }))
        } finally {
          setUpdating('')
          loadVersions() // 刷新版本状态
        }
      ...
      const renderVersionRow = (e) => {
        const v = versions[e.name]
        if (!v) return null
        if (v.builtin) {
          return React.createElement('div', { className: 'pc-verRow' },
            React.createElement('span', { className: 'pc-ver' }, '内置（由 DSH 托管，跟随主程序更新）'))
        }
        const parts = []
        if (v.local) parts.push('本地 v' + v.local)
        if (v.latest && v.source === 'npm') parts.push('最新 v' + v.latest)
        if (v.remoteHead && v.source === 'github') parts.push('锁定 ' + (v.latest || ''))
        const hasUp = v.hasUpdate === true
        const msg = updateMsg[e.name]
        const row = React.createElement('div', { className: 'pc-verRow' },
          React.createElement('span', { className: 'pc-ver' }, parts.length ? parts.join(' · ') : ''),
          hasUp && v.source === 'npm'
            ? React.createElement('span', { className: 'pc-updateBadge' }, '⬆ 有新版本')
            : hasUp
              ? React.createElement('span', { className: 'pc-updateBadge' }, '⬆ 远端有新提交')
              : null,
          v.source === 'npm' && hasUp
            ? React.createElement('button', {
                className: 'pc-updateBtn',
                disabled: updating === e.name,
                onClick: () => doUpdate(e.name),
              }, updating === e.name ? '更新中…' : '一键更新')
            : null,
          msg
            ? React.createElement('span', { className: msg.kind === 'ok' ? 'pc-noteSaved' : 'pc-updateErr' }, msg.text)
            : null,
        )
        return row
      }
